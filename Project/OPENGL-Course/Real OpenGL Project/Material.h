...
public:
	...
	void SetInt(const std::string& name, int val) { ints[name] = val; }
	int GetInt(const std::string& name) const { return ints.count(name) ? ints.at(name) : 0; }
	...
	const std::map<std::string, int>& GetInts() const { return ints; }
