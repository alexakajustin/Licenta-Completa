...
void main()								         
{
    ...
    // ============================================================
    // White-Water "Churn" (Foam generated by turbulence)
    // ============================================================
    // We calculate turbulence based on a combination of flow speed and normal variance
    float flow = time * 0.8;
    vec2 turbulentUV = TexCoord * vec2(4.0, 15.0) + vec2(0.0, -flow);
    float noise1 = texture(material_dudvMap, turbulentUV).r;
    float noise2 = texture(material_dudvMap, turbulentUV * 1.5 + vec2(0.2, -flow * 1.2)).g;
    float combinedNoise = (noise1 + noise2) * 0.5;
    
    // High-energy "milky" water base
    float turbulence = smoothstep(0.4, 0.8, combinedNoise);
    turbulence *= smoothstep(0.0, 0.2, depthDiff); // Fade near very shallow edges
    
    vec3 foamColor = vec3(0.95, 0.98, 1.0);
    finalBaseColor.rgb = mix(finalBaseColor.rgb, foamColor, turbulence * 0.7);
    finalBaseColor.a = mix(finalBaseColor.a, 1.0, turbulence * 0.5);

    // Shoreline White-Water (Stronger than lake foam)
    float foamDist = material_foamDistance == 0.0 ? 1.0 : material_foamDistance;
    if (depthDiff < foamDist) {
        float edgeFoam = (1.0 - smoothstep(0.0, foamDist, depthDiff));
        vec2 edgeNoiseUV = TexCoord * 5.0 + vec2(0.0, -time * 0.3);
        float edgeNoise = texture(material_dudvMap, edgeNoiseUV).r;
        float finalEdgeFoam = smoothstep(0.4, 0.6, edgeFoam * edgeNoise);
        
        finalBaseColor.rgb = mix(finalBaseColor.rgb, foamColor, finalEdgeFoam);
        finalBaseColor.a = max(finalBaseColor.a, finalEdgeFoam);
    }

    // ============================================================
    // Lighting & Specular Glint
    // ============================================================
    ...
    // Cinematic specular shimmer (Increased for rushing water)
    vec3 lightDir = normalize(directionalLight.direction);
    vec3 halfVector = normalize(viewDir - lightDir);
    float NdotH = max(dot(effectiveNormal, halfVector), 0.0);
    
    float shimmerFactor = pow(NdotH, 256.0) * 2.0 + pow(NdotH, 32.0) * 0.5;
    float sparkleNoise = random(FragPos * 15.0 + vec3(time * 0.2), 0);
    float sparkle = smoothstep(0.85, 1.0, sparkleNoise) * shimmerFactor * 5.0;
    ...
    // Edge alpha fade
    float edgeAlpha = smoothstep(0.0, 0.1, depthDiff);
