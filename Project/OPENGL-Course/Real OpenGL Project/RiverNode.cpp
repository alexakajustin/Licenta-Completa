...
void RiverNode::Deserialize(const json& j)
{
	...
	baseDepth = j.value("baseDepth", 2.0f);
	baseWidth = j.value("baseWidth", 3.0f);
...
void RiverNode::Execute(SceneManager& scene, NodeProgressCallback progress)
{
	...
	if (progress) progress(20.0f, "Seeding Springs...");
	...
	for (const auto& spring : springs)
	{
		...
		for (int step = 0; step < maxSteps; step++)
		{
			...
			if (!foundLower) break; // Reached a local minimum (pit)
			currX = bestX;
			currZ = bestZ;
		}
	}

	if (progress) progress(50.0f, "Calculating Flow...");

	// 3. Carve Rivers
	// We'll use the flowVolume to determine depth and width.
	// Wider rivers where multiple streams meet.
			...
			for (int rz = -radius; rz <= radius; rz++)
			{
				for (int rx = -radius; rx <= radius; rx++)
				{
					...
					if (nx >= 0 && nx < gridRes && nz >= 0 && nz < gridRes)
					{
						...
						{
							// Linear falloff for the trench shape
	...
	if (progress) progress(80.0f, "Carving Terrain...");
	...
	if (smoothPasses > 0)
	{
		...
		for (int p = 0; p < smoothPasses; p++)
		{
			...
			for (int z = 1; z < gridRes - 1; z++)
			{
				...
				{
					int idx = z * gridRes + x;
					// Only smooth if near a river to preserve mountains?
					// Or just smooth everything. For rivers, localized smoothing is better.
					if (flowVolume[idx] > 0)
					{
