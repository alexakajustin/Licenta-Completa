...
void main()
{
    ...
    float ambient = 0.6; // High ambient to ensure visibility
    
    vec4 texColor = vec4(1.0);
    if (useDiffuseTexture) {
        texColor = texture(theTexture, TexCoord);
        // Blend slightly with white if the texture is very dark
        float luma = 0.2126 * texColor.r + 0.7152 * texColor.g + 0.0722 * texColor.b;
        if (luma < 0.2) texColor.rgb = mix(texColor.rgb, vec3(1.0), 0.2);
    }
